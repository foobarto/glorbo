...
- **Ollama daemon/pull managers no longer crash on an abnormal child exit
  ...
  restart / advance the pull queue. Both managers now `Process.unlink/1` the
  child and rely on the monitor's `:DOWN`. Surfaced by pre-release adversarial
  review; regression tests inject a linked child and assert the manager
