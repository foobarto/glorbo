...
This is an append-only reconciliation record. Per GEP-1, an Accepted/Implemented GEP's body and decision log are not rewritten in place; the items below record where the shipped code diverges from the body above so future readers see the deviations without the original intent being edited away.

- **Smart-mode LLM classifier is a permanent stub (finding 1) — known-gap.** The GEP headlines a smart mode that "uses a cheap LLM call to classify unlisted hosts" (Goals; D3/D4), and the path is plumbed end to end except the LLM call itself. `Glorbo.Company.Supervisor.company_classifier_fun/2` builds the proxy classifier as `fn host, _port -> SmartClassifier.smart_classify(host, egress_config) end` (lib/glorbo/company/supervisor.ex:298) passing no `:classify_fun`, so `SmartClassifier.invoke_llm/3` falls back to `default_llm_classify/3` (lib/glorbo/network/smart_classifier.ex:221), which unconditionally returns `{:ok, :unknown, "-", "classifier not yet wired"}`. Every smart-mode host therefore resolves to `:unknown` and is denied — no LLM, no `smart_model`/agent-model dispatch (D4). The GEP's "Implemented" status overstates this dimension; wire `:classify_fun` to a real `Glorbo.Agent.Dispatch`-backed call or downgrade the smart-mode claim.

- **No egress audit events emitted (finding 2) — known-gap.** Goals require "every decision (allow/deny/pending) is audit-logged with host, agent, and reason," and §Audit events specifies six event types plus `AuditEntry.action_phrase/4` renderers. None exist: `grep` for `egress.allowed|egress.denied|egress.pending|egress.smart` across `lib/` returns nothing, and `audit_entry.ex` has no egress clause (lib/glorbo_web/components/audit_entry.ex:138-201). The proxy records decisions only via `Logger.info/debug` (e.g. lib/glorbo/network/proxy.ex:507-529) — runtime logs, not the append-only audit log. This is a real gap to the stated Goal and the entire §Audit events section.

- **No `/companies/:co/egress` LiveView or any §UI surfaces (finding 3) — known-gap.** Goals promise an egress LiveView (top-N hosts, denied tally, PubSub feed, agent filter) plus a sidebar row, a CompanyLive "egress · 24h" stat card, and an ApprovalQueue extension for pending-egress sentinels. None exist: `find lib -iname '*egress*'` finds no LiveView, and there is no egress route in `lib/glorbo_web/router.ex`. The only `egress` mentions in `lib/glorbo_web/` are an unrelated comment in `agent_live.ex:2111` and `dashboard_token.ex`. A director has no in-app way to see egress history or approve pending hosts. AgentLive's network dropdown does include `:proxy`, but the observability/approval UI surfaces are unbuilt.

- **Unlisted hosts get a permanent 403, not the specced sentinel + 503 (finding 4) — known-gap.** §Modes (strict/smart) and §Proxy daemon step 6 specify that unlisted hosts fall through to director approval: write a sentinel and reply `503 Unavailable Retry-After: 60`. The implementation never writes a sentinel and never sends 503/Retry-After. On `:unknown` (and the no-classifier path) the proxy sends `HTTP/1.1 403 Forbidden` (lib/glorbo/network/proxy.ex:511, :524, :536) with code comments deferring sentinel handling to a "Phase 3" that has not shipped (proxy.ex:478-482, 532-536). There is no GEP-19 approval-queue wiring anywhere under `lib/glorbo/network/`. Strict/smart unlisted hosts deny permanently with no approval path.

- **History cache is in-memory, 6h TTL, not wired into the production proxy, no persistence (finding 5) — spec_code_divergence / known-gap.** §History module + D6/D9 specify `Glorbo.Egress.History` persisting to `companies/<co>/state/egress-history.json` with a 30-day rolling window. The shipped `Glorbo.Network.History` is an ETS-backed GenServer that explicitly documents "No persistence. The cache is advisory" (lib/glorbo/network/history.ex:22) with `@default_ttl_ms 6 * 60 * 60 * 1_000` = 6 hours, not 30 days (history.ex:36). The proxy supports a `:history_server` opt (lib/glorbo/network/proxy.ex:741-767), but the production child spec in `Glorbo.Company.Supervisor.maybe_append_proxy/4` passes only `allowlist_fun` and `classifier_fun` (lib/glorbo/company/supervisor.ex:164-181) and no `:history_server`, so `build_history_fun/build_history_put_fun` resolve to nils and the cache is a production no-op. Both the module name (`Network.History` vs specced `Egress.History`) and the on-disk/TTL contract diverge; wire it in and add 30-day JSON persistence, or amend the GEP to record history as an unwired in-memory 6h advisory cache.
