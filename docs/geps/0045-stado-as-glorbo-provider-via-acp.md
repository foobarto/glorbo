...
This is an append-only record (GEP-1: an Accepted/Implemented GEP body is not rewritten in place; deviations between the specced design and the shipped code are recorded here).

- **D4 + Schema — `~/.local/share/stado` `rw` host bind → per-agent XDG relocation. Disposition: as-shipped (body is stale).** The GEP Schema (lines 119–128) and D4 (lines 251–262) mandate a second `[[auth_binds]]` with `host = "~/.local/share/stado"`, `mode = "rw"` so stado can persist its session JSONL trace to shared host state. The shipped `priv/providers/stado.toml` deliberately drops that rw host bind: it keeps only the single `~/.config/stado` `mode = "ro"` bind and adds an `[env]` block pointing `XDG_DATA_HOME` / `XDG_STATE_HOME` / `XDG_CONFIG_HOME` at `{workspace}/.local/{share,state}` and `{workspace}/.config`, relocating stado's data/state into the per-agent, per-company, rw workspace mount. The TOML comments cite security findings A-001 / B-006 / B-023 / B-026 as the rationale ("no cross-company read/write breach and no rw host bind"). This is a deliberate, security-motivated supersession of D4's `rw` host bind; the code is correct and the GEP body is the stale artifact. Side effect: D2 (lines 230–232) and D5's claim that the trace lives in host `~/.local/share/stado/sessions/` for host-side `stado audit verify` no longer holds — the trace lives inside the workspace mount.

- **D5 — "No session reuse across dispatches" vs shipped F6 ACP session resume. Disposition: as-shipped (body is stale / superseded).** D5 (lines 264–274) declares a single ACP session per dispatch with no reuse. The shipped code does the opposite via the F6 session-id persistence path: `lib/glorbo/cli/dispatcher.ex:111-116` derives a per-`(provider, task_id)` session file (`<workspace>/.glorbo/sessions/<provider>__<task_id>.txt`, `acp_session_file/3` at :965), reads any prior id (`read_acp_session_id/1` :984), threads it in as `:resume_session_id`, and after a successful dispatch writes the returned id back (`write_acp_session_id/2` :1040, called at :144). `lib/glorbo/cli/dispatcher/acp/client.ex:317-320` then sends `session/new {"resumeSession": "<UUID>"}` on the next dispatch, so the same task reuses one stado/codex/claude-code session across dispatches. Both read and write enforce a canonical-UUID guard (`uuid_shape?`, D8 at :992-1018, :1042) — non-UUID `sessionId` values (which stado's `--resume` CLI accepts but the `resumeSession` ACP param rejects with `-32602 invalid UUID length`) are dropped so a bad token can't wedge the agent, and the agent-writable session file is lstat/symlink-guarded (B-024). Covered by tests in `test/glorbo/cli/dispatcher_test.exs` and `test/glorbo/cli/dispatcher/acp/client_test.exs`. The reproduced behavior contradicts D5; the shipped F6 design (per-`(provider, task_id)` resume with UUID-shape validation) is authoritative.
