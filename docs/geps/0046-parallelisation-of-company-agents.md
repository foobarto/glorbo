...
This is an append-only record (GEP-1: an Accepted/Implemented GEP's body above is not rewritten; deviations from it are recorded here).

- **Test strategy items 3 & 4 / D8 — missing integration tests — known-gap (reproduced).** The GEP commits to two NEW integration tests as load-bearing (Test strategy §3–4, lines 295–307; D8, lines 442–457): `test/integration/concurrent_dispatch_test.exs` (real `Agent.Server`s under a real `Company.Supervisor`: two same-company agents with overlapping `agent.dispatch` audit timestamps, plus one agent at `max_concurrency = 3` producing 3 distinct `invocation_id`s before any `agent.complete`) and `test/integration/cross_company_concurrent_test.exs` (two companies dispatching simultaneously with overlapping cross-company `agent.dispatch` timestamps, codifying the GEP-2 invariant). Neither file exists in `test/integration/`, nor anywhere under a renamed path. The cross-process / cross-company concurrency path the GEP calls "the whole point" is therefore unverified by the committed suite. The unit layers (Test strategy §1–2) *are* present and substantive: `test/glorbo/agent/server_test.exs` exercises `max_concurrency = 3` producing 3 distinct in-flight invocations (lines 1306–1336) and pending_wake re-drain, and `test/glorbo/company/dispatch_semaphore_test.exs` covers acquire/release/at-cap-`:throttled`/crash-reclaim-via-`Process.monitor`/unbounded — but both run in-process, not across a real `Company.Supervisor`. Disposition: real gap to fix later (add the two integration tests), or amend §Test strategy/D8 with an honest append-only note descoping the integration layer.

- **Frontmatter `status: Implemented` overstates test-strategy completion — known-gap (reproduced, low severity).** The runtime code shipped in `17af5c5` is complete and faithful to the Design: `Glorbo.Company.DispatchSemaphore` exists (`lib/glorbo/company/dispatch_semaphore.ex`) and is wired as a sibling under `Company.Supervisor` (`lib/glorbo/company/supervisor.ex:115`); `Agent.Server` uses the `in_flight` map state machine; `Agent.Dispatch` acquires/releases the semaphore and propagates `{:throttled, :company_dispatch_cap}` (`lib/glorbo/agent/dispatch.ex:73,319–320,389,404,418`); per-invocation `stdout-<invocation_id>.log` is constructed at `dispatch.ex:1288`; the file-format schema keys are present and unit-tested. However, 2 of the 4 enumerated Test-strategy layers (both NEW integration tests, incl. the load-bearing cross-company one) never landed, so `Implemented` claims a test posture the suite does not hold. Disposition: land the two integration tests to make `Implemented` true, or add an append-only history entry here acknowledging the integration-test layer was descoped while the runtime and unit layers shipped.
