...
This is an append-only record (GEP-1: the body of an Accepted/Implemented GEP is not rewritten — deviations between this spec and the shipped code are captured here).

- **`fire` proposal subtype + fire auto-approval — known-gap.** The body specs `fire` as a first-class auto-approvable subtype (auto-approve table lines 198–213; D6 lines 526–541: "fire proposals are auto-approved when the target agent has no assigned tasks"). The code stops short: `Glorbo.Company.Router.maybe_auto_approve_hire/2` (`lib/glorbo/company/router.ex:1657`) only matches `%{"subtype" => "hire"}` and counts agents against `headcount_budget` — there is no `fire` clause, no assigned-task scan, and the GEP's own 2026-04-23 history entry (line 41–51) only claims `subtype: hire` auto-approve shipped. `fire` is still advertised in the FileSpec enum (`lib/glorbo/file_spec/proposal_md.ex:43-49`) and the MCP `create_proposal` tool, but a `fire` proposal always falls through to manual Director approval. Real gap to fix or to scope-down (amend D6 to `hire`-only) later.

- **`proposals` derived SQLite table — deferred (not yet shipped).** Migration step 4 (lines 353–354) says reindex indexes `proposals/*.md` into a SQLite `proposals` table; the GEP's own "Deferred" list (line 409) flags it as wave 2d. It is still deferred: `upsert_domain_row/2` (`lib/glorbo/filesystem/reindex.ex:380-391`) branches only on `company.md` and the agent `AGENT.md`/`agent.md` files and routes everything else through `true -> :ok`; `path_kind/2` (`reindex.ex:305-311`) buckets proposals as kind 2 ("other"), and no `schema "proposals"` module or migration exists. Proposals are read live off disk (`Glorbo.Company.Proposals.list/2`), never materialized.

- **Canonical proposal audit action names — spec/code divergence (as-shipped on the runtime path, stale in the sink moduledoc).** The body (lines 223–227, 258–260, 308–315) and the `ProposalsSink` moduledoc (`lib/glorbo/company/proposals_sink.ex:11-14`) both promise canonical `proposal.requested` / `.approved` / `.denied` / `.superseded`. In reality the sink's `classify/1` (`proposals_sink.ex:161-169`) deliberately emits observer-flavored `proposal.file_pending` / `_approved` / `_denied` / `_superseded` (its T12 rationale: it is a filesystem observer, not an authorizer). The authoritative flip path, `Glorbo.Company.Proposals.do_flip/7` (`lib/glorbo/company/proposals.ex:107,148`), does emit the canonical `proposal.approved` / `.denied` / `.superseded`; the Router emits `proposal.route` / `.rejected` / `.auto_approved`. The one name emitted nowhere in `lib/` is `proposal.requested` — it survives only as a moduledoc line and a `query_audit` doc example (`lib/glorbo_web/mcp/tools/query_audit.ex:27`), and the test that asserts it (`test/glorbo_web/mcp/tools_wave_b2_test.exs:437`) hand-seeds the audit row rather than exercising any emitter. The sink moduledoc claiming the canonical names is the concrete drift to correct.

- **Inbox surface (proposal cards in the Mine tab) — as-shipped (body stale; superseded by ProposalsLive).** §Inbox surface (lines 266–286), §Migration step 2 (line 347) and §Goals (lines 80–81) describe `InboxLive` rendering proposal cards with subtype badge, Approve/Deny/Archive buttons and a status-edit modal. That never landed in `lib/glorbo_web/live/inbox_live.ex` (zero proposal references). Instead a dedicated `GlorboWeb.ProposalsLive` (`lib/glorbo_web/live/proposals_live.ex`) shipped — it groups proposals by status and runs inline Approve/Deny (deny opens a reason modal) through `Glorbo.Company.Proposals.flip/4`, exactly as the 2026-04-23 "Implemented" history entry records (lines 38–51). The shipped UX is correct; the §Inbox surface section is the stale body text.
