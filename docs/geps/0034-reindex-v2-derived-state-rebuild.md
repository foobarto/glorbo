...
This is an append-only record (GEP-1: an Accepted/Implemented GEP's body above is not rewritten; deviations found during reconciliation against the shipped code are recorded here).

- **GEP-19 SQLite-index description is stale (corrected-ref / known-gap in GEP-19, not GEP-34).** GEP-34 D4 (this file, lines 278-289) and the shipped code rebuild `tasks_approval_state` purely by folding `approval.{requested,granted,denied}` audit JSONL — `Reindex.rebuild_tasks_approval_state/1` at `lib/glorbo/filesystem/reindex.ex:182,754`. GEP-19 (`docs/geps/0019-director-approval-workflow.md:119-123`) still claims the table is "fully rebuildable from sentinels + frontmatter," and lists columns `task_path PRIMARY KEY, agent_slug, status, denial_reason, …`. Both halves are wrong against the shipped schema: `lib/glorbo/tasks_approval_state.ex:30-40` has an Ecto `id` PK (not `task_path`) and the resolution column is `reason`, not `denial_reason`, plus a `company_slug` column GEP-19 omits. Confirmed; the fix belongs in GEP-19's append-only reconciliation block, not here — GEP-34's own body (lines 99-102, 278-289) already describes the audit-only replay correctly.

- **No live-write→wipe→reindex roundtrip test for the three GEP-34 gap tables (known-gap).** GEP-34 Goal #1 (this file, lines 119-122) requires reindex to produce a `glorbo.db` "byte-identical (modulo row IDs/timestamps) to one built by observing every event live." Confirmed not covered: the integration roundtrip (`test/integration/reindex_roundtrip_test.exs`) asserts only `companies`/`agents`/`reindex_state` and explicitly scopes the gap tables OUT (moduledoc lines 6-9, comment lines 74-75). The unit tests in `test/glorbo/filesystem/reindex_test.exs` (describe blocks at lines 194, 491, 676) drive reindex from hand-authored JSONL fixtures and assert only the result-map counts (`%{audit_events: N}`, `%{tasks_approval_state: N}`, `%{budgets: N}`) — none exercises the real writers (`Company.BudgetTracker`, `Approvals.Gate`, `Company.AuditLog`), snapshots the live-observed rows, deletes them, reindexes, and asserts the rebuilt rows equal the live ones. The load-bearing "byte-identical to live" invariant is therefore unverified end-to-end. Real gap to fill: add an integration test that emits a `budget.usage`, runs an approval lifecycle, and appends audit events through the production writers, then compares post-reindex rows to the live snapshot (modulo `id`/`inserted_at`/`updated_at`).
