...
This is an append-only reconciliation record (GEP-1: an Accepted/Implemented GEP's body is not rewritten in place; deviations from the body are recorded here instead).

- **§"Network policy" + §D4 keyword set and "advisory-only" proxy framing — as-shipped (body is stale).** The body declares `network: none`/`proxy`/`open` and calls `proxy` "advisory-only at the kernel layer … a netns + nftables hardening iteration is planned." The shipped enum is `network: ["loopback", "proxy", "full"]` (`lib/glorbo/file_spec/agent_md.ex:49`), and proxy is now kernel-enforced-or-refused, not advisory: `:proxy` dispatches launch bwrap under `pasta --splice-only` so the child gets a private netns where only the Glorbo-managed proxy port is reachable, and the dispatch fails outright if `pasta` is unavailable (`lib/glorbo/sandbox/bwrap.ex:66-76`, `:272-274`, `:920-947`). The keyword rename was made by GEP-23 D1; the kernel-enforced proxy by GEP-31 (the netns+nftables "planned" work GEP-5 anticipated). The code is correct; GEP-5's §"Network policy" and §D4 text is superseded — see GEP-23 D1 (keyword rename) and GEP-31 (proxy kernel-enforcement).

- **§"Shape of an invocation" baseline namespace flags — as-shipped (body is stale).** The body lists `--unshare-user-try … --unshare-cgroup-try` (line 82). The real composer emits the STRICT, non-`-try` variants with no silent kernel-absent fallback: `--unshare-user`, `--unshare-cgroup` (`lib/glorbo/sandbox/bwrap.ex:227-240`), and the test deliberately pins this ("B10: bare `--unshare-user` + `--unshare-cgroup`, never `-try` fallbacks", asserting both present and `refute "--unshare-user-try"` / `refute "--unshare-cgroup-try"`, `test/glorbo/sandbox/bwrap_test.exs:394-400`). The body also omits `--clearenv`, a real security control (wipes inherited BEAM/director env, then re-adds a minimal `--setenv` whitelist; `bwrap.ex:227-266`, test B11 at `bwrap_test.exs:402-409`). The `-try` mismatch is not cosmetic — it describes a soft sandbox that falls back to no-namespace, whereas the shipped sandbox refuses to degrade. Code is correct and stronger than the body says.

- **Web dashboard `build_sandbox_preview/3` argv drift — known-gap (real gap to fix later).** `lib/glorbo_web/live/agent_live.ex:2052-2062` builds the Director-facing `base:` flag list by hand and still shows the weak `--unshare-user-try`/`--unshare-cgroup-try` (lines 2055, 2059) and omits `--clearenv` — so it advertises a sandbox that does NOT match what `Glorbo.Sandbox.Bwrap.build_argv/1` actually emits (strict `--unshare-user`/`--unshare-cgroup` + `--clearenv`). This is a second, independent hardcoded copy of the argv that has diverged from the single source of truth and misleads the operator inspecting an agent. Fix: render the preview from `Bwrap.build_argv/1` output, or at minimum sync the `base:` list (drop the two `-try` suffixes, add `--clearenv`). The preview's `network` default is already correct (`"loopback"`, line 2049), so only the baseline-flag block drifted.
