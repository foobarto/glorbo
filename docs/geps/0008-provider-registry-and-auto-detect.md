...
Append-only record per GEP-1: an Accepted/Implemented GEP's body is not rewritten; deviations from the shipped code are recorded here rather than edited into §§1–13 above.

- **D1 / §7.4 step 3a / §9 "Reply file missing" — file-only reply contract (as-shipped; body stale).** GEP-8 D1 (lines 321–325) commits to "file-only reply, no stdout fallback" and explicitly rejects the hybrid "prefer file, fall back to stdout extraction" approach; §7.4 step 3a (line 227) and the §9 reply-file-missing row (line 266) likewise make a missing reply file a hard invocation failure with no fallback. The shipped Dispatcher does the opposite: `maybe_stdout_to_reply/4` (lib/glorbo/cli/dispatcher.ex:228–276, invoked at :203) captures stdout and materialises it into the reply path when the CLI exits 0 and no reply file was written. This reversal is not an undocumented drift — it is the explicit subject of **GEP-16 §7** ("Stdout → reply fallback", docs/geps/0016*.md:239–260, which quotes D1 verbatim as the contract being softened, and lists it in the frontmatter note at line 16). Disposition: the code is correct as-shipped per GEP-16; GEP-8's body is stale only in that D1/§7.4/§9 carry no forward pointer to the GEP-16 amendment. Body unchanged here by GEP-1; the GEP-16 forward link is the fix.

- **§8 / D3 — `glorbo doctor` provider table and `glorbo doctor --probe` (deferred / known-gap).** GEP-8 §8 (line 256) promises that "`glorbo doctor` prints a table covering the same data: name, installed?, path, version (if probed), usage parser, source (`builtin` or `user`)", and D3 (line 335) names `glorbo doctor --probe` as the manual version-probe trigger. Neither shipped. lib/glorbo/doctor.ex has zero references to `Registry`, `Provider`, `usage_parser`, or provider `source` (the 25 grep hits are all the unrelated `write_probe`/`do_write_probe` filesystem write-test helper at :234–:463); `@doctor_switches` (lib/glorbo/cli.ex:75) has no `--probe` flag. Provider visibility shipped only as the `/providers` LiveView (D16), backed by `Registry.list/0` — which lib/glorbo/cli/registry.ex:17 still documents as feeding "dashboard, doctor", a dangling reference to the unbuilt table. (A separate `glorbo detect-providers` command exists at lib/glorbo/cli.ex:268 but is GEP-32 localhost-port probing, not the GEP-8 PATH/version doctor table.) Disposition: deferred — specced, not built; record as not-yet-shipped. A future round either implements the table over `Registry.list/0` (+ `--probe` wiring `refresh_with_version_probe/0`) or this stands as the record that it was descoped to the LiveView.
