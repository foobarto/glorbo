...
This is an append-only record per GEP-1: the body of an Accepted/Implemented GEP is not rewritten, so divergences found during the reconciliation audit are recorded here instead.

- **§Sentinel shape (lines 245–247) — `classify_by_path` shadowing.** The GEP claims the `kind:` discriminator means `Glorbo.FileSpec.classify_by_path` "routes it through the new module's validator without ambiguity against generic inbox messages." That is false for `classify_by_path/1` as named. `Glorbo.FileSpec.InboxMessageMd` is registered at `lib/glorbo/file_spec.ex:93`, ahead of `PeerReviewRequestMd`/`PeerReviewFeedbackMd` at lines 99–100, and its `path_match?` regex `~r{/agents/[^/]+/inbox/[^/]+\.md\z}` (`lib/glorbo/file_spec/inbox_message_md.ex:13`) matches any `.md` in any agent inbox. Since `classify_by_path/1` is first-match-wins via `Enum.find` (`file_spec.ex:120`), it shadows both peer-review specs. Verified empirically: both `peer-review-release-01.md` and `peer-review-feedback-release-01.md` inbox paths classify as `Glorbo.FileSpec.InboxMessageMd`, not the dedicated modules. **Disposition: known-gap.** The claim is wrong specifically about `classify_by_path` (the path-based fallback used by the validator at `validator.ex:114,398` and the formatter at `formatter.ex:94` for unparsed/brand-new files); those callers would misclassify a not-yet-parsed peer-review sentinel as a generic `inbox-message/v1`. The impact is narrow because the registry's primary classifier is `classify_by_kind/1`, which reads the `kind:` frontmatter first (per the moduledoc, `file_spec.ex:16–21`) and correctly routes parsed sentinels — but the GEP named the one function that is in fact ambiguous, and no test asserts peer-review path classification (`test/glorbo/file_spec_test.exs` only counts modules and checks `kind` uniqueness). Suggested fix: register `PeerReviewFeedbackMd` then `PeerReviewRequestMd` before `InboxMessageMd` in `@specs` so their more-specific `peer-review-[…]`/`peer-review-feedback-[…]` regexes win the path-fallback (feedback before request, since its prefix is a superset), and add a regression test asserting `classify_by_path` on both paths.
  - **RESOLVED (2026-06-14).** Applied the suggested fix: `@specs` now lists `PeerReviewFeedbackMd`, then `PeerReviewRequestMd`, then `InboxMessageMd` (`lib/glorbo/file_spec.ex`), so `classify_by_path/1` routes a not-yet-parsed peer-review sentinel to its dedicated spec instead of the generic `inbox-message/v1`. Regression test added (`test/glorbo/file_spec_test.exs` — "peer-review sentinels are NOT shadowed by InboxMessageMd"): both sentinel paths classify to their own modules and a plain inbox message still classifies as `InboxMessageMd`. The §Sentinel-shape claim now holds for `classify_by_path` as written.
