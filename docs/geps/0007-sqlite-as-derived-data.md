...
- **Schema enumeration in the finding's evidence — corrected-ref.** The finding listed the live tables as ending in `provider_mode`; the actual table is `provider_models` (`20260423190000_create_provider_models.exs`, schema `lib/glorbo/provider_model.ex`). The current schema also includes two tables the evidence omitted — `chunk_vectors` and `memory_index_enabled` (GEP-32 memory index, migration `20260612120000_create_memory_index.exs`) — neither of which is mentioned in GEP-7 §"What SQLite holds" and both of which are likewise reindex-derived; worth folding into any future rewrite of that section.
