...
Append-only deviation record per GEP-1: the Accepted body above is not rewritten in place; the gaps a 2026-06-14 reconciliation audit found between this GEP and the shipped code are recorded here instead.

- **§Views table (12 web views/overlays → TUI) vs 7 shipped — deferred.** The §Design "drop-in parity checklist" (lines 881–894) maps 12 surfaces, and D8 declares all ship "in v1, no 'phase 2' slicing." The code ships exactly 7 list-views (`lib/glorbo/shell/views/{inbox,health,overview,agents,audit,chat,tasks}.ex`) — one per `C-c` chord letter (o/t/a/c/p/h/u). Skills, Goals, Agent-detail tabs (`Shell.Views.Agent`), the active/archived Inbox tabs, and the `M-x`/`Ctrl-k` Command-palette + `?` Keys-help overlays (`Shell.Overlays.*`) are absent (no module, no test). Disposition: **deferred** — the seven D10 chord-target views are the as-shipped v1 surface (the frontmatter history's "GEP-37 Phase 3 complete" note at line 595 reflects this); the §Views table's extra 5 entries and D8's "ship everything at once" claim are aspirational and not yet built. The otherwise-meticulous history never recorded that Skills/Goals/Agent-detail/Command-palette were dropped from the v1 cut.

- **`glorbo shell` "Phase 0 placeholder banner" help/status text vs the shipped 7-view runtime — as-shipped (body stale).** The runtime has worked since the Phase 2c/3 rounds: `Glorbo.Shell.Launcher.run/2` composes opts and invokes `TermUI.Runtime.run/1`, and `glorbo shell <company>` boots into a live `AppRoot` with `C-c` chord navigation across all 7 views (`lib/glorbo/shell.ex:71–73`). But the user-visible help still reads `[alpha] Interactive Director terminal (GEP-37 Phase 0)` (`lib/glorbo/cli.ex:732`), and `shell.ex` `help_text/0` STATUS block (lines 143–145) still says "Phase 0 (alpha) — CLI wired, `term_ui` dep installed, placeholder banner shown. Runtime + views land in subsequent rounds." Only the no-argument path (`args == []`, `shell.ex:65`) routes to `placeholder_banner_with_usage/0`; the with-argument path is fully live. Disposition: **as-shipped** — the runtime is correct; the "Phase 0" CLI/help/status strings are stale doc drift that should drop "Phase 0" and describe the shipped view set + chord nav (a code fix, not a GEP body change).

- **`Glorbo.Shell.{Keybindings, Theme, Renderer, InputReader}` + `Runtime.reduce/2` + `Renderer.frame/1` — corrected-ref.** The §Design and §Test-strategy name these as load-bearing single-source-of-truth modules/functions (Keybindings line 974; Theme 866–869; InputReader as supervision child #2, 820–824; `Runtime.reduce/2` 1095; `Renderer.frame/1` 1100). A grep across `lib/` + `test/` returns zero matches for all six names. The shipped reality: `term_ui` owns stdin raw-mode and cell-diff rendering, so InputReader/Renderer-as-pure-frame don't exist; keybindings live as per-view `event_to_msg/2` arms plus `AppRoot`'s chord dispatcher rather than a central `Keybindings`; there is no `Theme` module; and `Glorbo.Shell.Runtime` is a `:rest_for_one` event-accumulator GenServer (`handle_cast`/`handle_call`, `@max_events 256` at `runtime.ex:20`), not a pure `reduce/2` reducer. Disposition: **corrected-ref** — these are legitimate architecture evolution onto term_ui's Elm runtime; the GEP §Design (supervision tree, Keybindings/Theme) and §Test-strategy module names no longer name real modules and should be updated to the as-built shape.

- **4-layer test pyramid + ≥1 pty-backed E2E per view + 100% coverage on Runtime/Renderer/View.* — known-gap.** §Test-strategy mandates pure-reducer unit, render-snapshot, integration-against-fake-IO, and "E2E: at least one per view for v1" via `:exec`/`:erlang.open_port` with a pty under the existing E2E tag (lines 1095–1120). A grep across `test/glorbo/shell` for `pty|open_port|:exec|@tag :e2e|@moduletag :e2e|TermUI.Runtime.run|spawn_executable` returns zero hits; no test boots the real term_ui runtime (`launcher_test.exs` injects a recording `runner_fn` double, and the lone "term_ui" strings in `inbox_test.exs:362,379` are comments in a render-tree-flattening helper, not a runtime boot). The shipped suite is rich at the view-unit layer (event_to_msg/update/render-string assertions per view) but the pure-reducer/snapshot and pty-E2E layers the GEP specifies are absent. Disposition: **known-gap** — a real, fixable gap against both the GEP's own minimum bar and the project's 1-unit+1-E2E-per-feature discipline; at minimum one pty-backed E2E that boots `glorbo shell <fixture-company>` against a temp `~/.glorbo`, feeds `C-c` chord keys, and asserts rendered output.
