...
This is an append-only record (per GEP-1, an Accepted/Implemented GEP's body is not rewritten in place; deviations from the original spec are recorded here instead).

- **UI surface renamed: `GlorboWeb.ApprovalQueueLive` + `/companies/<co>/approvals` → `GlorboWeb.Live.InboxLive` (Mine tab) + `/companies/<co>/inbox`** — disposition: **corrected-ref**. The Problem and Design sections (`docs/geps/0019-director-approval-workflow.md:28`, `:101`, `:161-166`, `:275`, `:282`) and the Test-strategy section (`:185-188`) repeatedly name `ApprovalQueueLive` as "the dashboard" and the route `/companies/<co>/approvals`. Neither exists in the current tree: there is no `lib/glorbo_web/live/approval_queue_live.ex`, and the LiveView was folded into `InboxLive` (router comment at `lib/glorbo_web/router.ex:122-127`: "The standalone `/approvals` route was folded in here (backlog #14) — Inbox's Mine tab renders the same awaiting-approval sentinels with approve / deny / archive buttons"). The replacement is `lib/glorbo_web/live/inbox_live.ex`, whose `:mine` tab loads the same `awaiting-approval-<task_id>.md` sentinels (moduledoc at `:8`, loader comment at `:312`) and whose `approve`/`deny_confirm` events call `GlorboWeb.Actions.set_approval/4` (`:127-128`, `:147-150`) — the exact UI-direct path the GEP specs. So the protocol, sentinel contract, `assigned_to` swap, and audit shape are all unchanged; only the LiveView module name and route string moved. Read GEP-0019's `ApprovalQueueLive` / `/approvals` references as `InboxLive` (Mine tab) / `/companies/<co>/inbox`. (Residual stale references to the old name survive only in comments/moduledocs that explicitly document the fold: `lib/glorbo_web/live/inbox_live.ex:15`, `:312`; `lib/glorbo_web/components/sidebar.ex:293`; `lib/glorbo_web/time_format.ex:6`; `test/glorbo_web/components/sidebar_test.exs:9` — and the GEP's named test files `approval_queue_live_test.exs` / `approval_queue_integration_test.exs` no longer exist; the live UI is covered by `test/glorbo_web/live/inbox_live_test.exs`.)
