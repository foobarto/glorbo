...
This is an append-only reconciliation record (GEP-1: an Accepted/Implemented GEP's body is not rewritten; deviations are recorded here rather than edited into the sections above).

- **`company-template/v1` kind is unregistered in the FileSpec registry — known-gap.** The GEP's Migration section and D1 rationale claim the kind is "registered in GEP-25" and that the tree-on-disk approach keeps every template file covered by `glorbo validate` + `glorbo fmt` "for free." Reproduced: the `@specs` list in `lib/glorbo/file_spec.ex:71-101` has no `CompanyTemplateMd` (only `BenchmarkRunMd` at line 98), and there is no spec module on disk (`find lib -iname '*company_template*' -path '*file_spec*'` is empty; no `company-template/v1` string anywhere under `lib/glorbo/file_spec/`). The shipped template manifests do carry the kind (`priv/templates/companies/bench-softdev/template.md` opens with `kind: company-template/v1` + `min_glorbo_version: 0.0.4`), so files use a kind the validator/formatter cannot classify — `glorbo validate`/`glorbo fmt` silently skip them, contradicting D1's "for free" premise. Fix is to add and register a `Glorbo.FileSpec.CompanyTemplateMd`.

- **`mix glorbo.bench.verify` does not exist — deferred (not-yet-shipped).** Referenced as a real, load-bearing gate in three places: the Template-authoring workflow step 4, the Failure-modes row "Fixtures dir contains binary > 10 MB | `mix glorbo.bench.verify` rejects", and D7. Reproduced: `ls lib/mix/tasks/` lists only `gep.validate`, `glorbo.build_local`, `glorbo.docs.file_formats`, `glorbo.doctor`, `glorbo.kill`, `glorbo.migrate_tasks`, `glorbo.privacy_check`, `glorbo.release_formula`, `glorbo.release_guard` — no `bench.verify`; `grep -rln 'bench.verify\|Bench.Verify' lib` is empty. The advertised verification gate (manifest lint, assignee check, formatter idempotence, 10 MB fixture cap) is unimplemented.

- **Phase-A scaffolder has zero test coverage; the three named Phase-A tests do not exist — missing_tests / known-gap.** Test Strategy Phase A names `test/glorbo/templates/bench_templates_test.exs` (manifest validation), a scaffolder round-trip ("scaffold each bench template … every file passes `glorbo validate`"), and per-template variable rendering. Reproduced as absent: `find test -name bench_templates_test.exs` is empty and `grep -rln 'CompanyTemplate' test` returns nothing. The scaffolder it would cover does exist and ships — `Glorbo.CLI.Scaffold.CompanyTemplate.run/3` at `lib/glorbo/cli/scaffold/company_template.ex:35` — so this is untested production code, not vaporware. The only `bench-softdev` references in tests (`test/glorbo/benchmarks_test.exs:28,71,86`, `test/glorbo/benchmarks/orchestrator_test.exs:23`) are Phase-B run-manifest fixtures and orchestrator inputs; none scaffold a template, assert tree shape, exercise `{{ slug }}`/`{{ provider }}`/`{{ model }}` substitution, or test the `min_glorbo_version` refusal. The scaffolder round-trip and per-template rendering tests remain to be written.
