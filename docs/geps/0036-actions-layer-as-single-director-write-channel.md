...
This is an append-only record (GEP-1: an Accepted/Implemented GEP's body is not rewritten; deviations are captured here, not by editing the design above).

- **Task status/edit mutations bypass `Glorbo.Actions` via `TaskDefinition.write*` — known-gap (partly corrected-ref).** The GEP's Public API table (lines 197, 200) lists `Tasks.move/4` and `Tasks.update_status/4` as the single write channel for status/column changes, but neither function exists in `lib/glorbo/actions/tasks.ex` (only `create`, `trash`, `archive_to_history`, `reassign`, `record_peer_review_verdict`). Three LiveView handlers mutate task state directly through `Glorbo.TaskDefinition`: `kanban:move` at `kanban_live.ex:687` (`write/2`), `save_task` at `kanban_live.ex:481-482` (`write_frontmatter/2` + `write_body/2`), and the task editor at `task_live.ex:275` (`write_frontmatter/2`). The named-function refs are stale (corrected-ref: `Tasks.move`/`Tasks.update_status` were never built), and the underlying "single write channel" invariant is genuinely not held for task status/edit — a real known-gap. Mitigating note: all three handlers emit the audit entry inline (`emit_task_edit_audit` at `kanban_live.ex:1365`, `task_live.ex:320`) and one enforces the approval gate (`refuse_if_bypasses_approval_gate`), so the audit-gap consequence is largely closed even though validation/permission centralization is not.
