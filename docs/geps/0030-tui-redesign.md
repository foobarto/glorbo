...
This is an append-only record per GEP-1: the body of an Accepted/Implemented GEP is not rewritten in place; deviations discovered during the 2026-06-14 code reconciliation are logged here instead.

- **`confirm_modal` with typed-word gate — unimplemented (known-gap).** The GEP specs a destructive-confirm component with a typed-word gate as a Goal (`docs/geps/0030-tui-redesign.md:68`), a "New" deliverable (`:122-124`: `lib/glorbo_web/components/confirm_modal.ex`), and decision D10 (`:376-388`), to gate agent SIGKILL, company archive, and memory wipe. Reproduced: no such component or logic exists — grep for `confirm_modal` / `ConfirmModal` / typed-word / `gl-confirm` / `type-to-confirm` across `lib/`, `assets/`, `test/` returns zero hits, there is no `lib/glorbo_web/components/confirm_modal.ex`, and `core_components.ex` has no typed-word modal. All eight destructive-action sites instead use the browser-native `phx-click` + `data-confirm` prompt: agent delete/kill-dispatch/retire (`lib/glorbo_web/live/agent_live.ex`), channel archive (`channel_live.ex:313`), company emergency-stop/clear/backup (`company_live.ex:318,359,395`), and task delete (`components/task_detail_form.ex:171`). Disposition: **known-gap** — destructive actions are still confirmed (via the native one-click prompt), but the typed-word escalation D10 specified never shipped, so the GEP's `status: Implemented` overstates this slice. Fix later by either building the typed-word gate and wiring it into the kill/archive/wipe sites, or amending the GEP to drop the confirm_modal deliverable in favor of the as-shipped `data-confirm` approach. (Note for context, not part of this finding: the other three D10 "New" components — `command_palette`, `keys_overlay`, `kanban_task_drawer` — also have no standalone files under `lib/`; a fuller D10 reconciliation may be warranted.)
