...
This is an append-only record (GEP-1: an Accepted/Implemented GEP's body is not rewritten; deviations from the shipped code are recorded here rather than edited into the design above).

- **Finding 1 — Harness fixture seed never creates 4 of the 18 PAGES targets (known-gap).** The GEP's Tier-1/Tier-3 lists target per-entity LVs (`agents/ceo`, `tasks/inbox-01`, `tasks/inbox-01/chain`, `projects/inbox`), but `scripts/ui-baseline.sh` seeds only `./glorbo init --no-example` (line 105) + `./glorbo new company acme` (lines 113-114). The plain scaffold (`lib/glorbo/cli/scaffold/company.ex:109-153`) creates `company.md`, a `channels/general.md` stub, and otherwise *empty* `agents/`, `projects/`, and no tasks. So entries `06-agent`, `07-task`, `14-task-chain`, and `18-project` (ui-baseline.sh lines 52, 53, 60, 64) point at entities that never exist. Each of those four LVs `push_navigate`s away with a "not found" flash on a missing entity (`agent_live.ex:130-131` → company dashboard, `task_live.ex:86-87` → kanban, `task_chain_live.ex:76-77` → kanban, `project_live.ex:76-77` → company dashboard), so the capture lands on the redirect target, not the intended page — those four "baselines" are silently screenshots of the wrong route. Fix per the GEP's own suggestion: either drop `--no-example` and have ExampleCompany seed an `inbox` project + `inbox-01` task + a `ceo` agent, or add explicit `glorbo new`/task-seeding calls in `capture_pages`.

- **Finding 2 — `--init-from-fixture` harness flag does not exist (corrected-ref / doc_drift).** GEP §Harness (line 164) and §Failure modes (line 231) describe seeding "handled by an `--init-from-fixture` flag on the harness," and baking font+locale "into the harness's `--init-from-fixture` setup." A repo-wide search (`grep -rniE 'init.from.fixture|init_from_fixture'` over `.sh/.ex/.exs/.js`, excluding `node_modules`) returns zero matches — the flag was never built. The real seeding path is `glorbo init --no-example` + `glorbo new company acme` inside `capture_pages` (ui-baseline.sh lines 104-117), with no font/locale baking present. The two GEP references are stale naming for a flag that does not exist; the §Harness/§Failure-modes prose should read against the actual seed path. This is the same root cause as Finding 1 — the never-implemented fixture-seeding flag is exactly why the per-entity PAGES have no backing data.
