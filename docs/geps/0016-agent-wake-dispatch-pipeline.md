...
This is an append-only record (GEP-1: an Accepted/Implemented GEP's body is not rewritten; deviations from the body above are captured here).

- **§5 schema + D2 — `mode = "rw"` "reserved but unimplemented" (as-shipped; body is stale).** The body (lines 194–196, 343–345) claims `"rw"` is silently treated as `"ro"` because "auth dirs should never be rw-mounted." The shipped code does the opposite: `lib/glorbo/sandbox/bwrap.ex:402` — `defp bind_flag(host, sandbox, :rw), do: ["--bind", host, sandbox]` — emits a real writable bind, and `:ro` falls through to line 403's `--ro-bind`. This is intentional and tested: `test/glorbo/sandbox/bwrap_test.exs:208` (B5b) and `:347` (B8) assert `:rw` auth-binds emit `--bind`, not `--ro-bind`, including the `/workspace` sub-path case. The code is correct; D2's "reserved for future use" is the stale narrative. Security note that the body omits: a provider TOML can now request a writable host bind into the sandbox, so `[[auth_binds]] mode = "rw"` is a real capability grant — the argv slot is defended by `assert_valid_auth_bind_paths!/2` (bwrap.ex:405+, rejecting control bytes, relative/`..` paths, and critical system roots) rather than by clamping mode.

- **§4 + diagram line 292 + D4 — `cli_binary_binds/1` and same-host=sandbox `$PATH` dir-mounting (corrected-ref; the named function and strategy do not exist).** The body presents a verbatim `defp cli_binary_binds(%{resolved_path: path})` (lines 138–151) that returns `{dir, dir}` tuples mounting the symlink's parent dir and the target's enclosing dir at identical host/sandbox paths "so `$PATH` resolution inside the sandbox matches outside." No such function exists — `grep -rn cli_binary_binds lib/` returns nothing in code. The shipped strategy is materially different and the inverse of "mount parent dirs at the same path": `resolve_cli_binary/2` → `resolve_regular_binary/3` (`lib/glorbo/agent/dispatch.ex:1093–1124`) walks the symlink chain (depth-capped at 16, loop-guarded) to the final *regular file*, then `cli_binary_bind/1` (dispatch.ex:1089) binds **only that one file** to a *fixed, different* sandbox path `/tmp/glorbo-cli-<safe_name>-<basename>` (dispatch.ex:1078–1082; `cli_binary` sandbox path vs `host_cli_binary` host path threaded separately at dispatch.ex:947–948, 967). The source comment at dispatch.ex:1059–1062 states the rationale the GEP lacks: bind a single file, not parent dirs, "so agents cannot enumerate the binary's host-side parent directories" — a deliberate hardening that drops §4/D4's same-path `$PATH`-matching mechanism entirely. The §4 text, the diagram's `§4 cli_binary_binds` line (292), and D4's "`cli_binary_binds/1` runs on every dispatch using `System.find_executable/1` + `File.read_link/1`" should be read as describing `resolve_regular_binary/3` + `cli_binary_bind/1` instead.
