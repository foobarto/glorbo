...
This is an append-only record (per GEP-1, an Accepted/Implemented GEP body is not rewritten in place; deviations from the shipped code are recorded here instead).

- **Memory-index opt-in is SQLite-only — known-gap.** GEP-3's corollary (lines 55–58) forbids any SQLite field that can't be rebuilt from disk, and the invariant test at lines 59–60/196–199 expects `rm glorbo.db && glorbo reindex` to fully reconstruct state. The GEP-0058 semantic-recall opt-in violates this: `Glorbo.Memory.Index.enable/2` (lib/glorbo/memory/index.ex:65–74) writes only to the SQLite `memory_index_enabled` table, and grep confirms no markdown/JSONL/marker-file mirror (the sole other reference is the moduledoc note in lib/glorbo/memory/chunk_vector.ex:20). After a DB wipe, `Reindex.rebuild_memory_index/2` (lib/glorbo/filesystem/reindex.ex:209–239) finds zero enabled companies and silently leaves recall OFF until re-enabled. The `memory/index.ex` + `chunk_vector.ex` moduledocs were corrected in PR #62 to flag this caveat (they previously claimed "dropping glorbo.db loses nothing"), but the code-level invariant is still violated. Real gap to fix later: persist the opt-in to disk (e.g. a `memory:` key in company.md or a `cache/memory/<co>.enabled` marker) or explicitly carve memory-index opt-in out of the GEP-3 corollary.

- **Reindex contract step 1 ("drops and recreates the SQLite schema") — corrected-ref / as-shipped.** GEP-3 §"The reindex contract" step 1 (line 188) and the lines 59–60 corollary attribute schema (re)creation to `glorbo reindex`. The shipped split is different: migrations create the schema — `Ecto.Migrator.run/4` is invoked by `Glorbo.DB.Bootstrap` (lib/glorbo/db/bootstrap.ex:49), `glorbo migrate` (lib/glorbo/cli/migrate.ex:43–44), and the restore chain — while `Glorbo.Filesystem.Reindex.run/1` (lib/glorbo/filesystem/reindex.ex:126–196) is an MD5-incremental walker that `delete_all` + `insert_all` over a subset of already-existing derived tables (`audit_events`, `tasks_approval_state`, `budgets`, `provider_models`, memory chunks) and upserts `companies`/`agents` only where the file MD5 changed. It never creates a schema and never wipes the whole DB. The end-to-end invariant still holds (`rm glorbo.db` → boot/migrate recreates the schema, then reindex rebuilds the derived rows), but the GEP wording assigns the schema step to the wrong command. Correct the step-1 prose and the lines 59–60 example to reflect the migrations-create / reindex-rebuilds-derived-rows split (e.g. `rm glorbo.db && glorbo migrate && glorbo reindex`).
