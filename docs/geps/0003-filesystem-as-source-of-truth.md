...
- **Memory-index opt-in is SQLite-only — known-gap.** GEP-3's corollary (lines 55–58) forbids any SQLite field that can't be rebuilt from disk, and the invariant test at lines 59–60/196–199 expects `rm glorbo.db && glorbo reindex` to fully reconstruct state. The GEP-0058 semantic-recall opt-in violates this: `Glorbo.Memory.Index.enable/2` (lib/glorbo/memory/index.ex:65–74) writes only to the SQLite `memory_index_enabled` table, and grep confirms no markdown/JSONL/marker-file mirror (the sole other reference is the moduledoc note in lib/glorbo/memory/chunk_vector.ex:20). After a DB wipe, `Reindex.rebuild_memory_index/2` (lib/glorbo/filesystem/reindex.ex:209–239) finds zero enabled companies and silently leaves recall OFF until re-enabled. The code already owns this honestly (index.ex:39–46 names it a "Known gap (GEP-3 rebuildability)" tracked against GEP-58). Real gap to fix later: persist the opt-in to disk (e.g. a `memory:` key in company.md or a `cache/memory/<co>.enabled` marker) or explicitly carve memory-index opt-in out of the GEP-3 corollary.
