...
This is an append-only record (per GEP-1, an Accepted/Implemented GEP's body is not rewritten; deviations from the body are documented here rather than edited in place).

- **§2.3 / §2.4 — `path_access.granted` and `path_access.revoked` audit events at dispatch boundaries — known-gap.** The body says a `path_access.granted` audit fires at dispatch start and `path_access.revoked` at dispatch end. In code, `path_access.granted` is never emitted anywhere (no occurrence in `lib/`), and dispatch revocation calls the *unaudited* path: `dispatch.ex:349` (and `agent/server.ex:260`) call `Glorbo.PathGrantStore.revoke/3`, which only deletes the ETS row. The audited revoke that emits `path_access.revoked` does exist (`PathRequestGate.revoke/4`, `path_request_gate.ex:140-149`) but no dispatch-boundary caller uses it. Net: of the five-event trail, only `requested`/`approved`/`denied` ever fire; `granted` and `revoked` do not. Real gap — fix by routing dispatch revocation through `PathRequestGate.revoke/4` and emitting `path_access.granted` when `approved_paths` is non-empty at `dispatch.ex:927-931`.

- **§Failure modes — "no task exists" rejection (`path_access.invalid_task` + move to `history/`) — as-shipped (body is stale).** The body promises a task-existence check that rejects with a `path_access.invalid_task` audit and moves the outbox file to `history/`. The shipped flow has no task-existence gate: `Router.handle_outbox_path_request` (`router.ex:1438-1464`) validates only `kind`/`paths`/`reason`, and the gate's `validate_request/1` (`path_request_gate.ex:258-284`) checks task_id *format* only. Rejected requests are deleted via `File.rm(abs_path)` (`router.ex:1461`), not moved to `history/`, and the audit action is `path_access.rejected` (`router.ex:1456`), not `path_access.invalid_task`. The implementation is a reasonable simplification (a path request without a matching live task simply yields no grant at dispatch-build lookup); the GEP body's failure-mode prose is the stale part.
