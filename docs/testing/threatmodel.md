...
- **Outbox memory writes follow symlinks to host files** — `handle_outbox_memory_write/4` reads the outbox file with `File.read/1` and never checks that the path is a regular file. An untrusted agent can therefore create a symlink in its outbox (e.g., `outbox/memory/user_leak.md`) that points to another agent’s…
  *Paths:* `lib/glorbo/company/router.ex, lib/glorbo/agent/memory.ex`
- **Audit search allows XSS via unescaped audit log fields** — The commit adds audit log entries to search results and builds labels directly from `actor`, `action`, and `target` fields. Those fields can include attacker-controlled strings (for example `msg.to` in rejection audits allows arbitrary text besides control…
  *Paths:* `lib/glorbo/company/router.ex, lib/glorbo/search.ex, assets/js/app.js`
...
- **Search palette renders task titles as HTML (stored XSS)** — Glorbo.Search reads task titles directly from frontmatter and returns them as the `label` field in the JSON search API. The command palette then maps these labels into HTML via `innerHTML` without escaping. Because task titles can be authored by agents via…
  *Paths:* `assets/js/app.js, lib/glorbo/search.ex`
...
- **Unvalidated assignee path allows arbitrary inbox file writes** — In the new task modal, assigned_to is taken verbatim from params and forwarded to maybe_notify_assignee. That helper builds a filesystem path with Path.join and writes an inbox file if the directory exists. Because assigned_to is not validated as a slug, an…
  *Paths:* `lib/glorbo_web/live/kanban_live.ex`
- **Unvalidated assignee enables path traversal file writes** — The new task-assignment notification logic builds a filesystem path from the LiveView `assigned_to` parameter without validating it as a slug or restricting path separators. Because `Path.join/1` accepts absolute paths and `..` segments, a malicious dashboard…
  *Paths:* `lib/glorbo_web/live/kanban_live.ex`
...
- **Task comment assignee path traversal writes outside company** — The new task comment flow parses the task’s frontmatter and pulls the assigned_to value directly from the file. That value is then passed to write_director_mention, which builds a filesystem path with Path.join and creates inbox/mentions directories and files…
  *Paths:* `lib/glorbo_web/actions.ex`
- **Kanban open_task allows arbitrary project file reads** — The new KanbanLive open_task handler trusts the client-supplied "path" and calls File.read/1 after resolve_task_path/2, which only enforces a "projects/" prefix and absence of "..". It does not require a tasks/ path, a .md extension, or a regular file, and…
  *Paths:* `lib/glorbo_web/live/kanban_live.ex`
- **Kanban task creation follows symlinks to arbitrary paths** — The new Kanban LiveView builds an allowed project list from directory entries and trusts it when writing new tasks. Because `File.dir?/1` follows symlinks and there is no slug or lstat-based regular-file validation, an attacker who can place a symlink under…
  *Paths:* `lib/glorbo_web/live/kanban_live.ex`
- **Agent detail preview allows symlink-based host file reads** — AgentLive now loads the latest inbox/outbox markdown using File.read and embeds the preview in the LiveView response. Because inbox/outbox are writable by untrusted agents, a malicious agent can place a symlink (or hardlink) to any host file accessible to the…
...
- **Director actions follow symlinks to external paths** — The newly added GlorboWeb.Actions functions validate slugs and task path strings but do not verify that the resolved filesystem targets are regular files or that parent directories are not symlinks. set_approval/4 joins the task path and passes it to…
  *Paths:* `lib/glorbo_web/actions.ex, lib/glorbo/task_definition.ex`
