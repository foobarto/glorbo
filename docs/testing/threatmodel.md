...
Codex scan against commit `cc99146`, after closing 36 higher-severity
items across waves 1–3 (2026-04-22). **51 open** — 0 critical,
0 high, 5 medium, 28 low, 18 informational.

Format per row: **title** — short gist. *Paths:* touched files.
See `git log -- docs/testing/threatmodel.md` for the wave-1/2/3 closure log
and the original Codex CSV import (with per-finding URLs).

### Medium (constrained exploit — local access or misconfig) — 5
...
### Low (defense-in-depth / bounded DoS / integrity gaps) — 28
...
### Informational (correctness / UX — not a direct security gap) — 18
