...
| 01-02-T2 | 02 | 2 | FND-06 | — | `mix glorbo.doctor` runs all 5 checks, prints human table; `mix glorbo.doctor --json \| jq -e '.version \| test("^\\d+\\.\\d+\\.\\d+")'` exits 0 | integration | `mix test test/mix/tasks/glorbo.doctor_test.exs && mix glorbo.doctor --json \| jq -e '.checks \| length == 5'` | ✅ after T2 | ✅ green |
...
| 01-03-T2 | 03 | 3 | FND-03 | — | `./glorbo doctor --json` works on binary (argv dispatch end-to-end local smoke) | integration | `./burrito_out/glorbo_linux_x86_64 doctor --json \| jq -e '.version \| test("^\\d+\\.\\d+\\.\\d+")'` | ✅ after T2 | ✅ green |
| 01-03-T2 | 03 | 3 | FND-03 | — | Binary runs on host with no Erlang installed (clean Ubuntu 24.04 container smoke) | smoke | `podman run --rm -v $PWD/burrito_out:/b:ro ubuntu:24.04 /b/glorbo_linux_x86_64 doctor --json \| jq -e '.version \| test("^\\d+\\.\\d+\\.\\d+")'` → exit 0 | ✅ after T2 | ✅ green |
