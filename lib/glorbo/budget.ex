defmodule Glorbo.Budget do
  ...
  Each row aggregates LLM usage for one agent in one calendar month (UTC).
  Costs are stored in integer cents to avoid floating-point drift in SUM
  aggregations (D-26 confirmed).

  The `budgets` table has a composite unique index on `{agent_slug, year_month}`
  so Plan 02's BudgetTracker can upsert with atomic `inc:` on the conflict
  target (see RESEARCH Pitfall 4 for the safe Ecto pattern).
  """
  use Ecto.Schema
  ...
  def changeset(%__MODULE__{} = budget, attrs) do
    budget
    |> cast(attrs, [:agent_slug, :year_month, :prompt_tokens, :completion_tokens, :cost_usd_cents])
    |> validate_required([:agent_slug, :year_month])
    ...
    |> unique_constraint([:agent_slug, :year_month],
      name: :budgets_agent_slug_year_month_index
    )
  end
end
