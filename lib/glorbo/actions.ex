defmodule Glorbo.Actions do
  @moduledoc """
  Canonical Director write-actions layer (GEP-36). Every call resolves
  to a filesystem mutation (markdown append, frontmatter rewrite, or
  wake-request write) followed by an append-only `AuditLog` event — in
  that order, so a write failure never leaves an orphan audit trail
  ...
  ## Functions
    ...
    * `post_task_comment/4` — Director comments on a task thread;
      audit action `task.comment`.
    ...
    * `wake_agent/4` — Director writes `agents/<slug>/state/wake-request.md`
